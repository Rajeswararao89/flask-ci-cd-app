...
def detectTestCommand() {
    ...
    if (fileExists('pytest.ini') || fileExists('setup.py') || fileExists('requirements.txt')) return 'pip install pytest flask -q && mkdir -p test-results && python -m pytest tests/ --junitxml=test-results/junit.xml -v'
